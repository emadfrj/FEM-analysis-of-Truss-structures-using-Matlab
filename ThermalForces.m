...
for i=1:n
    e=Thermal(i,1);%number of element that has thermal load
    ...
    %transform matrix
    Tthermal=[-c;-s;c;s];
    ThermalForce = E(e,2)*A(e,2)*alpha(e,2)*Thermal(i,2)*Tthermal;
    Fth(([node1*2-1 node1*2 node2*2-1 node2*2])) = ThermalForce +Fth(([node1*2-1  node1*2 node2*2-1 node2*2])) ;
end